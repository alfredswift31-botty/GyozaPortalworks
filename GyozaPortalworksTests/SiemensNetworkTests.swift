...
struct SiemensNetworkTests {
    ...
    @Test func editingOperationsBuildASealIn() throws {
        ...
        let start = try #require(network.insertContact(operand: "%I0.0", at: .start(path: rung)))
        let stop = try #require(network.insertContact(.normallyClosed, operand: "%I0.1", at: .after(element: start)))
        let motor = try #require(network.insertCoil(operand: "%Q0.0", at: .after(element: stop)))
        let branch = try #require(network.openBranch(at: .start(path: rung)))
        let hold = try #require(network.insertContact(operand: "%Q0.0", at: .start(path: branch)))
        let closed = network.closeBranch(branch, onto: start)
        #expect(closed)
        let box = try #require(network.insertEmptyBox(at: .after(element: motor)))
